...
export default function EditProjectPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchProjectAndCategories = async () => {
      try {
        ...
        setValue("detailedDescription", data.detailedDescription);
        setValue("client", data.client);
        setValue("tools", data.tools?.join(", "));
        setValue("tags", data.tags?.join(", "));
        setValue("url", data.url);
        setValue("featured", data.featured);
        
        setImages(data.images?.length > 0 ? data.images : (data.image ? [data.image] : []));
      } catch (error) {
        console.error("Failed to load project or categories", error);
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      await axios.patch(`/api/projects/${params.id}`, projectData);
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-white/20">
        ...
        <p className="text-luxury uppercase tracking-widest">Loading Project...</p>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto space-y-12">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-4">
          ...
          >
            {isSubmitting ? "Updating..." : <><Rocket size={18} /> Update Project</>}
          </button>
        </div>
      </div>
